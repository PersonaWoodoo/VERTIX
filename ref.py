    ...
    if written:
        ...
        try:
            await message.bot.send_message(
                chat_id=inviter_id,
                text=(
                    f"{CUSTOM_EMOJI_JOIN} {invitee_mention} вступил по вашей ссылке!\n"
                    f"💰 Начислено: +{REF_REWARD:,} {CURRENCY_NAME}".replace(",", " ")
                ),
                parse_mode="HTML"
            )
        except Exception:
...
@router.message(F.text.lower().in_({"👥рефералы", "рефералы", "👥 рефералы", "реферал"}))
async def cmd_ref(message: Message):
    ...
    text = (
        ...
        f"{CUSTOM_EMOJI_INVITED} Приглашено: <b>{invited_count}</b>\n\n"
        f"🔗 <b>Ваша ссылка:</b>\n"
        f"<code>{ref_link}</code>\n\n"
        f"🎁 За каждого приглашённого: <b>+{REF_REWARD:,} {CURRENCY_NAME}</b>".replace(",", " ")
    )
